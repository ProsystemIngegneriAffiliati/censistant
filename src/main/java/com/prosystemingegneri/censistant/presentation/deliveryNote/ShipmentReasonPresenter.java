...
@Named
@ViewScoped
public class ShipmentReasonPresenter implements Serializable{
    @Inject
    ...
    public String saveShipmentReason() {
        try {
            service.saveShipmentReason(shipmentReason);
        } catch (EJBException e) {
            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", ExceptionUtility.unwrap(e.getCausedByException()).getLocalizedMessage()));
            return null;
        }
        
        return "/secured/deliveryNote/shipmentReasons?faces-redirect=true";
    }
    
    public void detailShipmentReason() {
        if (id == 0)
            shipmentReason = new ShipmentReason();
        else
            shipmentReason = service.readShipmentReason(id);
    }
